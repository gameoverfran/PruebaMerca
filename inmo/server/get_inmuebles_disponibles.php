...
try {
    ...
    if (is_null($headers["Authorization"]) == false){
        ...
        if (strcmp($tokenUnlock["rol"],"a") == 0  ){
            $sentencia = $bd->prepare("select * from inmueble where proyecto_inmobiliario_idproyecto_inmobiliario is null");
            $sentencia->execute([]);
            $inmuebles = $sentencia->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode($inmuebles, JSON_UNESCAPED_UNICODE); 
